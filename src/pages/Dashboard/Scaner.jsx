...
const Scaner = () => {
  ...
  const filteredResults = (() => {
    ...
    data.forEach((item) => {
      ...
      const scanType = item.scanType || (item.data && item.data.scanType);
      ...
      switch (scanType) {
        ...
        case "SQLInjectionScanner":
          ...
          if (item && item.results && Array.isArray(item.results)) {
            item.results.forEach((data) => {
              ...
              if (
                !data.vulnerable_parameters ||
                data.vulnerable_parameters.length === 0
              ) {
                const resultItem = {
                  type: "sql_injection",
                  status: data.status,
                ...
                if (
                  !filterText ||
                  Object.values(resultItem).some((value) =>
                    value
                      .toString()
                      .toLowerCase()
                      .includes(filterText.toLowerCase())
        ...
        default:
          ...
          if (Array.isArray(item)) {
            ...
            item.url &&
            (item.vulnerable_parameters || item.payloads || item.databases)
          ) {
            // Assume SQL injection if URL and vulnerability-related fields exist
            if (
              !item.vulnerable_parameters ||
              item.vulnerable_parameters.length === 0
            ) {
              results.push({
                type: "sql_injection_secure",
                url: item.url,
            ...
            const subdomains =
              item.subdomains || (item.data && item.data.subdomains) || [];
            const liveSubdomains =
              item.live_subdomains ||
              (item.data && item.data.live_subdomains) ||
              [];
  ...
  return (
    <div className="min-h-screen main-container bg-[#0E1427] text-white p-6">
      <PageTitle title={`${cleanedPath} Scanning`} desc="" />
      <div className="main-container">
        <div className="bg-[#040C1F] p-4 mb-4 rounded-md border border-[#4C566A]">
        ...
        {/* Logs Section */}
        <div className="mt-4 bg-[#040C1F] rounded-md border border-[#4C566A] p-4">
          ...
          <div className="h-40 overflow-y-auto text-sm bg-[#0F172A] p-3 rounded-md border border-[#4C566A]">
            {logs.length === 0 ? (
              <p className="text-gray-500">
                No logs yet. Start a scan to see logs.
              </p>
            ) : (
              logs.map((log, index) => (
                <p
        ...
        <div className="mt-4 bg-[#040C1F] rounded-md border border-[#4C566A] p-4">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <h3 className="text-[#04D2D2] font-bold mb-3 text-lg">
              Scan Results
            </h3>
